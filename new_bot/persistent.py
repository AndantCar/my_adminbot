...
SELECTING_ACTION, ADDING_PAYMENT, ADDING_SELF, DESCRIBING_SELF = map(chr, range(4))
...
def start(update, context):
    ...
    text = 'You may add. To abort, simply type /stop.'
    buttons = [[
        InlineKeyboardButton(text='Lista de pagos', callback_data=str(ADDING_PAYMENT)),
    ...
    if context.user_data.get(START_OVER):
        ...
        logger.info(update.callback_query.answer())  # jijijijiiji que disi.while.Kelly
        update.callback_query.edit_message_text(text=text, reply_markup=keyboard)
    else:
        update.message.reply_text('Hola, soy AdminBot y aqui podras guardar toda la informacion que quieras administrar')
...
def select_level(update, context):
    ...
    text = 'Aqui podras administras listas de pagos'
    buttons = [[
        InlineKeyboardButton(text='Añadir pago', callback_data=str(PARENTS))
    ], [
        InlineKeyboardButton(text='Mostrar datos', callback_data=str(SHOWING)),
...
def select_gender(update, context):
    ...
    buttons = [[
        InlineKeyboardButton(text='Añadir pago: ', callback_data=str(MALE))
    ], [
        InlineKeyboardButton(text='Show data', callback_data=str(SHOWING)),
        InlineKeyboardButton(text='Back', callback_data=str(END))
    ]]
